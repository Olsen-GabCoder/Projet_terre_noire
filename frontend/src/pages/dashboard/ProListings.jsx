...
const ProListings = () => {
  ...
  const handleToggle = async (listing) => {
    try {
      ...
      toast.success(listing.is_active ? 'Offre désactivée.' : 'Offre réactivée.');
      fetchListings();
    } catch (err) { toast.error(handleApiError(err)); }
  };
  ...
  return (
    <div className="author-space">
      <div className="author-space__header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          <h1 className="author-space__title"><i className="fas fa-tags" style={{ color: 'var(--color-secondary)' }} /> Mes offres de service</h1>
          <p className="author-space__subtitle">{listings.length} offre{listings.length !== 1 ? 's' : ''} publiée{listings.length !== 1 ? 's' : ''}</p>
        </div>
        {!showForm && (
          <button className="as-cta" onClick={() => setShowForm(true)}>
            <i className="fas fa-plus" /> Nouvelle offre
          </button>
        )}
      </div>
      ...
      {showForm && (
        <div className="as-card" style={{ marginBottom: '1.25rem' }}>
          <div className="as-card__header">
            <h2 className="as-card__title"><i className="fas fa-edit" /> Nouvelle offre</h2>
          ...
          <div className="as-card__body">
            <form onSubmit={handleSubmit}>
              <div className="ob-form__grid">
                <div className="ob-form__field"><label>Type de service *</label>
                ...
                <div className="ob-form__field"><label>Tarification *</label>
                ...
                <div className="ob-form__field ob-form__field--full"><label>Titre de l'offre *</label>
                  <input type="text" name="title" value={form.title} onChange={handleChange} required placeholder="Ex: Correction complète de manuscrit" />
                </div>
                <div className="ob-form__field ob-form__field--full"><label>Description *</label>
                  <textarea name="description" value={form.description} onChange={handleChange} rows={3} required placeholder="Décrivez votre service en détail..." />
                </div>
                <div className="ob-form__field"><label>Prix de base (FCFA) *</label>
                  <input type="number" name="base_price" value={form.base_price} onChange={handleChange} min="0" required />
                </div>
                <div className="ob-form__field"><label>Délai moyen (jours)</label>
                  <input type="number" name="turnaround_days" value={form.turnaround_days} onChange={handleChange} min="1" />
                </div>
                <div className="ob-form__field"><label>Langues</label>
                  <input type="text" name="languages" value={form.languages} onChange={handleChange} placeholder="Français, Anglais..." />
                </div>
                <div className="ob-form__field"><label>Genres</label>
                  <input type="text" name="genres" value={form.genres} onChange={handleChange} placeholder="Roman, Essai..." />
                </div>
              </div>
              <div className="ob-form__actions" style={{ marginTop: '1rem' }}>
                <button type="button" className="dashboard-btn" onClick={() => setShowForm(false)}>Annuler</button>
                <button type="submit" className="as-cta" disabled={saving}>
                  {saving ? <><i className="fas fa-spinner fa-spin" /> ...</> : <><i className="fas fa-check" /> Publier</>}
                </button>
              </div>
            </form>
          </div>
      ...
      {listings.length === 0 && !showForm ? (
        <div className="as-card">
          <div className="as-card__body as-empty">
            ...
            <p>Créez votre première offre de service pour être visible sur la marketplace.</p>
            <button className="as-cta" onClick={() => setShowForm(true)} style={{ marginTop: '1rem' }}>
              <i className="fas fa-plus" /> Créer une offre
            </button>
          </div>
        </div>
      ) : !showForm && (
              ...
              <div className="as-card__body" style={{ padding: '1rem 1.25rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '1rem', flexWrap: 'wrap' }}>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.25rem', flexWrap: 'wrap' }}>
                    ...
                    <span className={`ob-badge ${listing.is_active ? 'ob-badge--ok' : 'ob-badge--off'}`}>{listing.is_active ? 'Active' : 'Inactive'}</span>
                  </div>
                ...
                <button className="dashboard-btn" onClick={() => handleToggle(listing)} style={{ fontSize: '0.8rem' }}>
                  <i className={`fas fa-${listing.is_active ? 'eye-slash' : 'eye'}`} /> {listing.is_active ? 'Désactiver' : 'Réactiver'}
                </button>
              </div>
            </div>
          ))}
